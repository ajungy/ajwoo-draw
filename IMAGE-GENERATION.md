...
Use https://draw.ajwoo.com, or run `npm install`, then `npm run dev`, and open the local address (normally http://127.0.0.1:5173).
...
Prompt, Copy, and Connect are enabled on the public site. Public API requests go directly from the browser to fixed OpenAI/Google endpoints using the user’s memory-only key; Draw has no hosted relay or shared API bill. Browser/network restrictions may block provider requests; users can disconnect and use Copy instead. Local development continues to use the same-origin loopback relay.
